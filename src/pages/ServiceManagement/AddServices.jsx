...
const AddServices = ({ closeModal, fetchDataFromApi, accessToken }) => {
    ...
    const fetchAllServiceNames = async () => {
      try {
        const response = await axios.get(
          "http://ec2-13-233-113-80.ap-south-1.compute.amazonaws.com:5000/admin/getServicesCatogory"
        );
        const serviceNames = response.data.payload.map(
  ...
  const handleChange = (e) => {
    ...
    if (name === "name") {
      ...
      const existingServiceNames = allServiceNames.map((serviceName) =>
        serviceName.trim().toLowerCase()
      ); // Trim and convert each existing service name to lowercase
  ...
  return (
    <div className="bg-white dark:bg-slate-800 p-4 shadow-lg rounded-sm">
      ...
      <div className="mb-4">
        <h2 className="text-2xl text-MAROON font-bold mb-4">
          Create New Service
        </h2>
        <form onSubmit={handleSubmit}>
          <div className="mb-0">
            ...
            {serviceNameError && (
              <span className="text-red-500 text-sm  ">{serviceNameError}</span>
            )}
            <input
              type="text"
              id="name"
          ...
          <div className="flex justify-center">
            <button
              ...
              className={`bg-MAROON hover:bg-DARK_CREAM text-white font-bold py-2 px-4 rounded ${
                !!serviceNameError || serviceData.name.trim() === ""
